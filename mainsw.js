...
async function StayAlive() {
  ...
  var wakeup = setInterval(() => {
	...
	CLUTlog(`(DEBUG StayAlive) ----------------------- time elapsed: ${age}`);
    ...
    if (alivePort == null) {
      ...
      alivePort.onDisconnect.addListener((p) => {
        if (chrome.runtime.lastError) {
			CLUTlog(`(DEBUG StayAlive) Disconnected due to an error: ${chrome.runtime.lastError.message}`);
        } else {
			CLUTlog(`(DEBUG StayAlive): port disconnected`);
        }
        alivePort = null;
      });
    }
    ...
    if (alivePort) {
      ...
      if (chrome.runtime.lastError) {
        CLUTlog(`(DEBUG StayAlive): postMessage error: ${chrome.runtime.lastError.message}`);
      } else {
        CLUTlog(`(DEBUG StayAlive): "ping" sent through ${alivePort.name} port`);
      }
    }
    //lastCall = Date.now();
  }, 25000);
